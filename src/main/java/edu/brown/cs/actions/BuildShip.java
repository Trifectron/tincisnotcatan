...
/**
 * Action responsible for building a ship during game play (Seafarers). Modeled
 * on {@link BuildRoad}: a ship occupies a {@link Path}, but may only be placed
 * on a water/coastal path connected to the player's network.
 *
 */
public class BuildShip implements Action {
  ...
  public Map<Integer, ActionResponse> execute() {
    ...
    if (!(_player.numShips() > 0)) {
      ActionResponse resp = new ActionResponse(false,
          "You do not have any more ships", null);
      return ImmutableMap.of(_player.getID(), resp);
    }
    if (!_path.canPlaceShip(_player)) {
      ActionResponse resp = new ActionResponse(false,
          "You cannot build a ship in that location", null);
      return ImmutableMap.of(_player.getID(), resp);
    }
    if (_mustPay) {
      if (_player.canBuildShip()) {
        ...
        ActionResponse resp = new ActionResponse(false,
            "You cannot afford to build a ship", null);
        return ImmutableMap.of(_player.getID(), resp);
      }
    }
    _player.useShip();
    ...
    ActionResponse toPlayer = new ActionResponse(true, "You built a Ship",
        null);
    String message = String.format("%s built a Ship", _player.getName());
